...
class Artificial_intelligence(Data_extraction):
    ...
    def features_extract_test3(self) : 
        
        data = pd.read_csv(os.path.join(self.path_to_data_dir,
                           "data_dp45_test3.csv"), header=None, index_col=None)
        
        X = data.iloc[:, 1]
        y = data.iloc[:, 0]  # array  of signal_shape
    ...
    def traj_for_test_test3(self,  z, i=1):
        
        traj_dipole_list = self.extract_dipole_value_traji(
        range(self.traj), z=z) 
        
        X_test = traj_dipole_list[i][10][:,1] - traj_dipole_list[i][10][0,1]
        
        alt = self.extract_alt( z0 = z)
    ...
    def traj_for_test(self, i=6, segment_width=10):
        ...
        path = os.path.join(self.path_to_data_dir,
                            'data_traj_z_4\\data_T1_P_1_DP13_traj_{}.csv'.format(i))
        path_alt = os.path.join(self.path_to_data_dir,
                                'data_traj_z_4\\alt_T1_P1_traj_{}.csv'.format(i))
...
# %%
    ...
    # plt.hist(y_train, np.arange(np.min(y_train.ravel())-0.2,np.min(y_train.ravel())+0.4,0.01))
    # plt.xticks(np.arange(np.min(y_train)-0.2,np.min(y_train)+0.4,0.2))
    ...
    plt.title(
        "evolution de l'altitude (traj :{}) en fonction de la position, modèle :{} ".format(i, name))
    plt.legend()
    plt.figure()
