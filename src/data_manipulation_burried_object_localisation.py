...
class Data_extraction:
    ...
    def plot_cartographie_v2(self, num_dipole_list, z=5,  axis_x=True):
        ...
        for dp, Idint in enumerate(current_all):
            plt.figure(dpi=200)
            plt.title("cartographie : T = {},  dipoles : {}, Alt : {}cm".format(
                self.TEST, self.dipole[dp], z))
    ...
    def plot_altitude_echosondeur(self, list_num_traj, ech=True, axis_x=True):
        ...
        for e, i in enumerate(list_num_traj):
            # fig = plt.figure()
            data_i = pd.read_csv(os.path.join(self.path_altitude_echosondeur,
                                 list_traj_altitude[i]), header=None, index_col=None, dtype="float64")
            data_i = np.array(data_i.iloc[2:, :], dtype=np.float64)

            tsec = [data_i[:, :4], data_i[self.ech_cut0[e]:self.ech_cutf[e], :4]][ech]@np.array(
                [[3600], [60], [1], [1e-3]])  # equivalent a if ech=True : on ecahntillonne
            altitude = (data_i[:, 4], data_i[self.ech_cut0[e]:self.ech_cutf[e], 4])[
                ech]                                    # elsee : on echantillonne pas
            ...
            if self.save_altitude_data:
                ...
                df_altitude = pd.DataFrame(np.concatenate(
                    (tsec, data_i[:, 4].reshape(-1, 1)), axis=1))
