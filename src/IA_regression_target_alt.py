...
class Artificial_intelligence(Data_extraction) : 
    ...
    def features_extraction(self):
        ...
        self.alt = X.shape[0]
        self.traj_num = 17
        self.dp_n = int(X.shape[1]/( self.signal_shape*self.traj_num)) # nombre de dipole
            ...
            for i in range(self.traj_num) :
                for k in range(int(self.signal_shape/segment_width)):
                    ...
                    for j in range(self.dp_n) :
                        Li.extend(list(X[alt][i][j][segment_width*k:segment_width*(k+1)]))
...
if __name__ == '__main__' :
    ...
    dict_model = { "LR" : model1, "BR" : model2, "DT" : model3, "RF" : model4, "SVR" : model5 }
